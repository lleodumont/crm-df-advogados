...
export default function Pipeline() {
  ...
  const uniqueSources = Array.from(new Set(leads.map(l => l.utm_source).filter(Boolean)));
  const uniqueMediums = Array.from(new Set(leads.map(l => l.utm_medium).filter(Boolean)));
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              Filtrar
              {(filters.classification !== 'all' || filters.campaign !== 'all' || filters.utm_source !== 'all' || filters.utm_medium !== 'all') && (
            ...
            <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  >
                    ...
                    {uniqueSources.map((src, idx) => (
                      <option key={idx} value={src as string}>{src}</option>
                    ))}
                  </select>
                </div>
              )}
              ...
              {uniqueMediums.length > 0 && (
                <div>
                  ...
                  >
                    <option value="all">Todas</option>
                    {uniqueMediums.map((med, idx) => (
                      <option key={idx} value={med as string}>{med}</option>
                    ))}
                  </select>
                </div>
              )}
              ...
              <div className="flex gap-3 pt-4">
                <button
                  onClick={() => {
                    setFilters({ classification: 'all', campaign: 'all', utm_source: 'all', utm_medium: 'all' });
      ...
      <div className="flex gap-4 overflow-x-auto pb-6 px-2 md:px-6 snap-x snap-mandatory" style={{ minHeight: 'calc(100vh - 200px)', alignItems: 'flex-start' }}>
        {stages.map((stage) => {
          ...
          if (filters.campaign !== 'all') {
            columnLeads = columnLeads.filter(l => l.campaign === filters.campaign);
          }
          if (filters.utm_source !== 'all') {
            columnLeads = columnLeads.filter(l => l.utm_source === filters.utm_source);
          }
          if (filters.utm_medium !== 'all') {
            columnLeads = columnLeads.filter(l => l.utm_medium === filters.utm_medium);
          }
