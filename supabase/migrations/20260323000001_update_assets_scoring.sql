-- Atualiza a função de scoring para incluir novos tipos de bens no Pilar Patrimônio
...
BEGIN
  ...
  tmp := 0;
  IF v_assets_range IN ('acima_1m', 'acima_5m', '1m_5m') THEN tmp := tmp + 20;
  ...
  -- Tipos que somam +10 pts
  IF public.has_token(v_assets_types, 'empresa_cotas') OR public.has_token(v_assets_types, 'empresas') THEN tmp := tmp + 10; END IF;

  -- Tipos que somam +7 pts
  IF public.has_token(v_assets_types, 'imovel_financiado') THEN tmp := tmp + 7; END IF;
  IF public.has_token(v_assets_types, 'imovel_proprio') THEN tmp := tmp + 7; END IF;
  IF public.has_token(v_assets_types, 'investimentos') THEN tmp := tmp + 7; END IF;
  IF public.has_token(v_assets_types, 'previdencia') THEN tmp := tmp + 7; END IF;
